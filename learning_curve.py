...
Both modes train on growing slices and evaluate each slice on ONE fixed
...
for _p in ("gbt", "rocket_sim", "common"):
...
def run_nn(args):
    ...
    _write_r2(args, sizes, n_test, rows, list(TARGETS),
              "Neural surrogate learning curve", "nn_learning_curve")


# ── Shared R^2 output ────────────────────────────────────────────────────────
def _write_r2(args, sizes, n_test, rows, target_names, title, stem):
    os.makedirs(args.out_dir, exist_ok=True)
...
def main():
    ap = argparse.ArgumentParser(description="Model accuracy vs. dataset size (nn / classifier).")
    ap.add_argument("--mode", choices=["nn", "classifier"], default="nn")
    ...
    {"classifier": run_classifier, "nn": run_nn}[args.mode](args)
